...
function ExerciseDetail({
  ...
  return (
    <div className="space-y-4">
      ...
      <Select value={selectedExerciseId} onValueChange={(value) => onExerciseChange(value ?? '')}><SelectTrigger className="h-12 w-full rounded-[16px] bg-white px-4 font-bold dark:bg-[#1c1c1c]"><SelectValue placeholder="Selecciona un ejercicio">{definition ? `${exerciseDefinitionLabel(definition)} · ${definition.equipment} · ${definition.unit}` : undefined}</SelectValue></SelectTrigger><SelectContent>{definitions.map((exercise) => <SelectItem key={exercise.id} value={exercise.id}>{exerciseDefinitionLabel(exercise)} · {exercise.equipment} · {exercise.unit}</SelectItem>)}</SelectContent></Select>
