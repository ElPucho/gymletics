...
export function CalendarScreen({
  ...
  const completedDates = data.sessions.filter((session) => session.status === 'completed').map((session) => new Date(`${session.date}T12:00:00`));
  const missedDates = data.calendarMarks.filter((mark) => mark.status === 'missed').map((mark) => new Date(`${mark.date}T12:00:00`));
  const pendingDates = data.calendarMarks.filter((mark) => mark.status === 'pending').map((mark) => new Date(`${mark.date}T12:00:00`));
  const excusedDates = data.calendarMarks.filter((mark) => ['rest', 'vacation', 'illness', 'excused', 'rescheduled'].includes(mark.status)).map((mark) => new Date(`${mark.date}T12:00:00`));
  ...
  const streak = useMemo(() => {
    const marks = [...data.calendarMarks].filter((mark) => ['completed', 'missed'].includes(mark.status)).sort((a, b) => b.date.localeCompare(a.date));
    let count = 0;
  ...
  return (
    <div className="pb-28">
      <ScreenHeader title="Calendario" subtitle="Planificación y cumplimiento" />
        ...
        <Card className="rounded-[24px] bg-white py-3 ring-black/6 dark:bg-[#1c1c1c] dark:ring-white/10">
          <CardContent className="px-2">
            <Calendar
              ...
              modifiers={{ completed: completedDates, missed: missedDates, pending: pendingDates, excused: excusedDates }}
              modifiersClassNames={{
                completed: '[&_button]:after:absolute [&_button]:after:bottom-1 [&_button]:after:size-1 [&_button]:after:rounded-full [&_button]:after:bg-emerald-500',
                missed: '[&_button]:text-red-600 [&_button]:line-through',
                pending: '[&_button]:after:absolute [&_button]:after:bottom-1 [&_button]:after:size-1 [&_button]:after:rounded-full [&_button]:after:bg-amber-500',
