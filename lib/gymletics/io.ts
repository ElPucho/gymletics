...
import { bodyFatKg, bodyMuscleKg } from './body-metrics';
...
export async function exportPdf(data: GymleticsData) {
  ...
  if (latestBody) document.text(`Última medición: ${formatWeight(latestBody.weight)} kg · grasa ${formatWeight(bodyFatKg(latestBody) ?? 0)} kg · músculo ${formatWeight(bodyMuscleKg(latestBody) ?? 0)} kg`, 16, y + 18);
  y += 32;
  document.setFont('helvetica', 'bold');
  document.setFontSize(14);
